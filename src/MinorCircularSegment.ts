...
/**
 * Should not be used to represent minor circular segments with zero or otherwise very small areas
 * (to avoid division operations that could cause number overflow).
 */
export class MinorCircularSegment {
  readonly arcAngle: number;

  /**
   * The arc of the minor circular segment is interpreted to go from the first point of the chord
   ...
   *
   * Throws if the provided chord and arc length do not result in a minor circular segment.
   */
  constructor(private chord: Chord, private moreProps: { arcLength: number }) {
    ...
    if (arcLength <= chordLength) {
      throw new Error('Arc length is too small.');
    } else if (arcLength >= Math.PI * chordLength / 2) {
      throw new Error('Arc length is too big.');
    }

    // the angle between the arc and chord where they meet (within the minor circular segment)
    // (pi / 2 seems to work well as an initial guess)
    let incidentAngle = Math.PI / 2;
    ...
    for (let i = 0; i < iterations; i++) {
      let y = ((arcLength / chordLength) * Math.sin(incidentAngle)) - incidentAngle;
      let yPrime = ((arcLength / chordLength) * Math.cos(incidentAngle)) - 1;
